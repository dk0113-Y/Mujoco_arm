...
这些 Benchmark-0 seed 不是 Calibration、Development 或 Held-out Test 划分。尤其在默认配置三种模式均为 fixed 的情况下，pilot 不能代表随机场景性能，任何 smoke/pilot 结果都不能形成正式算法结论。正式划分由下述 Evaluation Protocol v1 单独管理。
...
正式问题定义、成功语义、五个生产型指标、三类数据集和版本规则见 `docs/evaluation_protocol_v1.md`；B1 参数 allowlist 与冻结流程见 `docs/b1_calibration_and_freeze_policy.md`。独立协议配置启用 random pick/place/physics，不改变通用默认配置。

```powershell
# 重建并核对确定性 split（正常验证只使用 --validate-only）
C:\ai_workspace\venvs\Mujoco_arm\Scripts\python.exe scripts\generate_protocol_splits.py --protocol configs\protocols\evaluation_protocol_v1.toml --validate-only

# 只 reset 环境并读取任务元数据；不建 Renderer、不运行控制器或 IK
C:\ai_workspace\venvs\Mujoco_arm\Scripts\python.exe scripts\inspect_protocol_distribution.py --protocol configs\protocols\evaluation_protocol_v1.toml

# 两个独立 smoke seed；记录 B1-Vision，并保留成对 Oracle 诊断
C:\ai_workspace\venvs\Mujoco_arm\Scripts\python.exe scripts\run_calibration.py --protocol configs\protocols\evaluation_protocol_v1.toml --seeds-file configs\splits\evaluation_protocol_v1\calibration_smoke.txt --output-dir outputs\evaluation_protocol_v1_calibration_smoke
```
...
Evaluation Protocol v1 已建立 Calibration/Development/Held-out 划分和描述性生产代理指标，但本任务没有执行正式 Calibration、Development 或 Held-out Test，也没有冻结 B1、提供显著性检验/置信区间、最终排名或 B2/学习算法结论。
